        ...
        <div className="toggle-div">
          <div className="git-task-div">
            <Button.Group size="mini" compact>
              <Button name="solutions" onClick={this.handleToggler} color={toggler === 'solutions' ? 'teal' : 'grey'}>Solutions</Button>
              <Button.Or />
              <Button name="tasks" onClick={this.handleToggler} color={toggler === 'tasks' ? 'teal' : 'grey'}>Tasks</Button>
            </Button.Group>
          </div>
          <div className="show-toggle-div">
            <ul className="solution-ul" style={{ display: `${this.state.toggler === 'solutions' ? 'block' : 'none'}` }}>
              {this.state.solutions.length !== 0 ? this.state.solutions.map((solution, i) => ( 
                <a key={i} target="_blank" rel="noopener noreferrer" href={ solution.Entry.link.includes('http') ? solution.Entry.link : `https://${solution.Entry.link}` }>
                  <li className="list-titles">
                    {solution.Entry.text}
                    <p className="li-text li-text-right">-- Submitted on: {new Date(solution.Entry.time).toLocaleDateString()}</p> 
                  </li>
                </a>
                  ))
            ...
            <ul className="task-ul" style={{ display: `${this.state.toggler === 'tasks' ? 'block' : 'none'}` }}>
              {this.state.tasks.length !== 0 ? this.state.tasks.map((task, i) => (
                <a key={i} href={`task/${task.Hash}`}>
                  <li className="list-titles">
                    ...
                    <p className="li-text li-text-right">Created on: {new Date(task.Entry.time).toLocaleDateString()}</p>
                  </li>
                </a>
                  ))
